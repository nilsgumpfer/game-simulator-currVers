...
// .94 se 15-10-15 pr�ft auf ungespeicherte �nderungen
// .95 se 15-10-22 undo
...
public class CodeWindow extends JFrame implements ActionListener, DocumentListener, ExecutorListener {
	...
	private static final String exeNametext = "Name f�r Programm";
	private static String commandsFromFiletext;
	private static final String showColorChooserText = "Farbw�hler";
...
//		}
		...
		// "Name f�r Programm (generierte C-Datei)");
		// Utils.addMenuItem(this, menuCompile, exeCommandtext,
		// "Befehl zum Ausf�hren");
		...
		String className = "lib.jserver.XSend" + messages.getLocale().getLanguage().toUpperCase();
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (cmd.equals(runText)) {
			...
			if (!text.matches(regex)) {
				JOptionPane.showMessageDialog(this, "Bitte keine Umlaute oder �hnliches im Namen..", "Wrong Name",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			if (codeDB.hasSnippet(text)) {
			...
			codeInput.setFont(font);
			messageField.setFont(font);
			// updateInfoLabel(); // ver�ndert gr��e der Fenster ???
			...
			String a = JOptionPane.showInputDialog(this, "Name f�r Programmdatei, bei VS noch in mscomp.bat eintragen",
