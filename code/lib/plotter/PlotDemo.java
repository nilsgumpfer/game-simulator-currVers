...
public class PlotDemo {
	...
	public void rechne8() {
		...
		// 10 Texte �bereinander
		for (int i = 0; i < 10; i++) {
			// Schreibe Text zentriert um Position 0,i
			// R�ckgabe ist das neu angelegte Textobjekt
			TextObject t = plotter.setText(text, 0, i);
			// nimmt den ersten Font, leite davon eine Variante in Gr��e 20 ab
			float size = 12 + random.nextInt(16);;
			Font testFont = fonts[(int) (Math.random() * fonts.length)]
					.deriveFont(size);
			// Zus�tzliche Ausgabe des Fontnamens
			//System.out.println( testFont.getFontName());
			// setzte den neuen Font f�r den Text
			t.setFont(testFont);
			// jetzt noch Farbe �ndern
